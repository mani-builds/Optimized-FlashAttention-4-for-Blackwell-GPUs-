...
def flash_v1_kernel(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor,
    ...
    O = kernel.launch_fwd_kernel(Q, K, V)
    return O
...
print(o1[:1,:1,:10, :10])
# torch.cuda.synchronize()
# assert torch.allclose(o, o1, atol=1e-3), "Naive and Pytorch Outputs do not match"

o2 = flash_v1_kernel(q,k,v)

print(o2[:1,:1,:10, :10])
# print(type(o2))
assert torch.allclose(o1, o2, atol=1e-3), "Pytorch and CUdA kernel outputs do not match"
